...
pub fn resource_derive(tokens: proc_macro::TokenStream) -> proc_macro::TokenStream {

	let input: DeriveInput = syn::parse(tokens).unwrap();
	let name = &input.ident;

	if let Data::Struct(_) = input.data {} else {
		panic!("can only generate resource for structs");
	}

	return proc_macro::TokenStream::from(quote! {
		impl dirty::ecs::Resource for #name {}
	});
